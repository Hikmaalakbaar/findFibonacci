...
#include <string> // meng aktifkan tipe data string. tipe data string >> string nama = "Hikmal";
//#incude <conio.h>

using namespace std; // cin :: std >> nama :: std << endl; // dihapus
...
struct Tugas {
    string nama; 
    bool prioritas; // TIPE DATA BOOLEAN
    bool dilaksanakan;
};


void tambahTugas(Tugas toDoList[MAX_TUGAS], int &jumlahTugas) { // & pada variabel jumlahTugas itu untuk referensi, agar value pada fariabel tersebut bisa diubah ditempat
    Tugas tugasBaru; // pendeklarasian variabel
    cout << "Masukkan nama tugas baru: "; // Makan Malam
    cin.ignore();  
    getline(cin, tugasBaru.nama);


    char pilihanPrioritas; // Pendeklarasian
    do {
        cout << "Apakah tugas ini memiliki prioritas? (y/n): "; // y, n
    ...
    if (jumlahTugas == 0) {
        ...
        for (int i = 0; i < jumlahTugas; ++i) {
            ...
            if (toDoList[i].prioritas) {
                cout << "[Urgent] ";
            } else {                                /*1. [urgent] [Selesai] Makan Malam*/
                cout << "[Biasa] ";
            }
